...
public class Outils {
    ...
    public static double[][] convoluer(ImageProcessor ip, Masque masque) {
        ...
        // On utilise l'effet miroir pour gérer les bords
            ...
            for (int x = 0; x < lig; ++x) {
                for (int v = -rayon; v <= rayon; ++v) {
                    for (int u = -rayon; u <= rayon; ++u) {
                        ...
                        if (Y < 0 || Y >= col) Y = y - v;
                        resultat[x][y] += matrice[X][Y] * masque.get(u, v);
                    }
                }
            }
        }
    ...
    public static void afficheMatrice(double[][] mat, String titre,
        ...
        if (normaliser) {
            ...
            for (int y = 0; y < mat[0].length; y++) {
                for (double[] aMat : mat) {
                    if (aMat[y] > max) max = aMat[y];
                    if (aMat[y] < min) min = aMat[y];
                }
            }
    ...
    public static void afficherMatrice(double[][] matrice) {
        ...
        for (int y = 0; y < hauteur; ++y) {
            for (double[] aMatrice : matrice) System.out.print(aMatrice[y] + "\t");
            System.out.println();
        }
    }
